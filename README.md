...
MovHint will allow you to receive custom recommendations on what movies to watch, basen on your personal tastes, and what you've already seen.
...
Adjust the login buttons so that they correspond to your own credentials, for both Facebook Login:
```js
...
The source code for MovHint is available on [GitHub](https://github.com/Stefano-Palumbo/MovHint), which will allow you to host your own version of the App. Before doing so, though, it's necessary to configure a few settings, since no database, nor hosting, is provided.
### Getting Started
...
### Server Setup
MovHint uses two separate servers , a [NodeJS](https://nodejs.org/en/) for the main API, and a secondary [Python Flask](https://palletsprojects.com/p/flask/) instance for managing recommendations.
...
Afterwards, define the necessary variables in it, while making sure **NOT** to mistype the names:
...
First, initialize the database with the popular movies by running the following command:
```sh
npm run first
```
...
- [Google App Engine](https://cloud.google.com/appengine), Provided by Google, and has a free tier and can be used for the NodeJS Server

- [Python Anywhere](https://www.pythonanywhere.com/), Provided by Anaconda, and has a free tier that is enough to deploy the Flask Recommendations service

- [CloudFlare Pages](https://www.cloudflare.com/products/pages/), Provided by Cloudflare, and has a free tier that allows deployment of static Websites to their global CDN.
