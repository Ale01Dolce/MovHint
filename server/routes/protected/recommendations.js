...
router.post("/recommendations", async (req, res, next) => {
    ...
    responseData.filter(elem => {
        ...
        return (
            elem.adult === preferences.adult && // Remove adult movies, if necessary
            elem.runtime <= preferences.length &&  // Remove movies going over the max length
            elem.genres.some(genre => preferences.genres.includes(genre.id)) && // Remove movies without any preferred genre
            (preferences.isPopular ? elem.vote_average > 7.00 : true) && // Remove movies that are lowly rated, if necessary
            (preferences.easilyWatchable ? providerLength !== undefined : true) && // Remove movies without any streaming service, if necessary
